...
b<-t(b)
colnames(b) <- c(1,2,3,4)  #CHANGE THESE BELOW TO REP THE ACTUALY RANGE OF CATEGORIES
...
tabrr <- cbind(t(dstan), srexpected)   #sprichness
...
#########################################################################
###Vellend terr sp. richness: 
#observed= tr, expected = tsrexpected
tabtr <- cbind(t(vct), tsrexpected)   #sprichness
...
################
# Visualization Step 1: create the final dataframes.  Step 2: see ChiSqfigures.R
...
mi.dat<-data.frame(label=factor(row.names(tabrmi), ordered=TRUE,
                   ...
                   marine.area.sq.km=imarine.area,
                   unifsites=floor(unif.dist * 430324), # Number of study sites? 443000
                   r.value=mi.r.values)
